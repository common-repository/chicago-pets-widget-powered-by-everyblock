<?php
/*
Plugin Name: Chicago Pets Widget Powered By Everyblock 
Plugin URI: http://tirespider.com/
Description: Add a Chicago Pets Widget to a post or page.
Version: 2.0.2
Author: Jeff S
...
class ChicagopetsWidget extends WP_Widget {

	public static $metro = "chicago";
	public static $schema = "pets";
	
	public function ChicagopetsWidget()
	{
		$widget_ops = array('classname' => 'ChicagopetsWidget', 'description' => 'Embeds a Chicago Pets Widget' );
		$this->WP_Widget('ChicagopetsWidget', 'Chicago Pets Widget', $widget_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("ChicagopetsWidget");') );

function display_chicago_pets_widget($atts) {
	$atts = extract(shortcode_atts( array('width' => '300', 'height' => '500'), $atts, 'display_chicago_pets_widget'));
	
	$code = "<iframe src=\"" . plugins_url( 'widget.php', __FILE__ ) . "?metro=" . ChicagopetsWidget::$metro . "&schema=" . ChicagopetsWidget::$schema . "&width=" . $width . "&height=" . $height . "&url=" . ABSPATH . "\" width=\"". $width . "\" //height=\"" . $height . "\" frameborder='0' scrolling='no'></iframe>";
	 
	return get_html_for_embed_chicago_pets(stripslashes($code));
}

function get_html_for_embed_chicago_pets($embedCode) {
	return '<div class="widget_block">' . stripslashes($embedCode) .'</div>';  
}

add_shortcode('display_chicago_pets_widget', display_chicago_pets_widget);
